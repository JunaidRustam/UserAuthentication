...
const { generateToken } = require("../db&token/token");
...
exports.login = async (req, res) => {
  try {
    ...
    if (!user) {
      return message.sendErrorResponse(res, 401, "Invalid Email");
    }
    // Compare passwords
    const isPasswordMatch = await bcrypt.compare(password, user.password);
    if (!isPasswordMatch) {
      return message.sendErrorResponse(res, 401, "Invalid  password");
    }
